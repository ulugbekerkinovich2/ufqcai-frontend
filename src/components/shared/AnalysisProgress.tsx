...
/**
 * Tahlil jarayoni vizual indikatori.
 * Tahlil ~60-120s davom etadi, biz oddiy taymerga ko'ra qadamlarni yoritamiz:
 * 0-3s:    parsing
 * 3-15s:   RAG (qonun qidiruvi)
 * 15-90s:  AI tahlil
 * 90s+:    natija saqlash
 */
export function AnalysisProgress({ startedAt }: { startedAt?: string }) {
  ...
  // Qaysi qadam aktiv
  let active = 0;
  if (elapsed >= 3) active = 1;
  if (elapsed >= 15) active = 2;
  if (elapsed >= 90) active = 3;
  ...
  return (
    <div className="card p-10 animate-fade-in">
      ...
      <div className="text-center mb-8">
        ...
        <div className="mt-2 inline-flex items-center gap-2 text-[13px] text-ink-muted">
          <span className="inline-block h-1.5 w-1.5 rounded-full bg-accent animate-pulse-soft" />
          {t("progress.elapsed")}: <span className="font-mono tabular-nums text-ink">{mm}:{ss}</span>
          <span className="text-ink-subtle">· {t("progress.eta")}: ~01:30</span>
        </div>
      </div>
        ...
        {steps.map((s, i) => {
          ...
          return (
            ...
            >
              ...
              {!pending && (
                <span className="text-[11px] text-ink-subtle tabular-nums shrink-0 mt-1">
                  {done ? "✓" : `${elapsed}s`}
                </span>
              )}
            </div>
